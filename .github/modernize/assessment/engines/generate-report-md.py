...
def _discover_language_inputs(report_dir):
    ...
    Returns an ordered list of (language_label, kind, path) where kind is
    'appcat' (report-java.json / report-dotnet.json) or 'jsts'
    ...
    """
    suffixed = [
        ("Java", "appcat", "report-java.json"),
        (".NET", "appcat", "report-dotnet.json"),
        ("JavaScript / TypeScript", "jsts", "report-javascript-typescript.md"),
    ]
    found = [(label, kind, os.path.join(report_dir, name))
             for (label, kind, name) in suffixed
...
def main():
    ...
    # Render each language independently, then stitch the fragments under one heading
    # per language (mirrors the local pipeline's render-then-stitch — no merged JSON).
    multi = len(inputs) > 1
    sections = []
    for (label, kind, path) in inputs:
        if kind == "appcat":
            fragment = render_appcat_report(report_dir, path)
        else:
            fragment = render_jsts_report(report_dir, path)
        if not fragment:
            continue
        if multi and label:
