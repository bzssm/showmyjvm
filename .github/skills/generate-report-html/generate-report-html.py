...
def _render_appcat_panel_fragment(report_dir, report_json_path):
    ...
    # Language token used to scope this panel's ids (tab id, row ids) so a
    # multi-language report can host several issue panels without collisions.
...
def _discover_language_inputs(report_dir):
    ...
    Returns an ordered list of (kind, path) where kind is 'appcat'
    (report-java.json / report-dotnet.json) or 'jsts'
    (report-javascript-typescript.md). Falls back to the legacy bare names
    (report.json / js-assessment-report.md) so older single-language layouts
    still render. Mirrors the sibling generate_report_md.py discovery."""
    suffixed = [
        ("appcat", "report-java.json"),
        ("appcat", "report-dotnet.json"),
        ("jsts", "report-javascript-typescript.md"),
    ]
    found = [(kind, os.path.join(report_dir, name))
             for (kind, name) in suffixed
             if os.path.isfile(os.path.join(report_dir, name))]
    if found:
        return found

    # Legacy single-language fallback: bare filenames.
    bare_json = os.path.join(report_dir, "report.json")
    if os.path.isfile(bare_json):
        return [("appcat", bare_json)]
...
def _compose_html(report_dir, inputs):
    ...
    for kind, path in inputs:
        if kind == "appcat":
            fragments.append(_render_appcat_panel_fragment(report_dir, path))
        else:
