...
// Per-panel current target, keyed by the owning .tab-panel's id. Each language
// panel computes and stores its own target independently so filters/effort/donuts
// in one language tab never read or clobber another's state.
var _currentTargetByPanel = {};

function _panelOf(el) {
  return el ? el.closest('.tab-panel') : null;
}

function toggleMultiSelect(btnOrEl) {
  var el = btnOrEl && btnOrEl.classList && btnOrEl.classList.contains('multi-select') ? btnOrEl : (btnOrEl ? btnOrEl.closest('.multi-select') : null);
  if (!el) return;
  var panel = _panelOf(el) || document;
  panel.querySelectorAll('.multi-select.open').forEach(function(ms) { if (ms !== el) ms.classList.remove('open'); });
...
function getMultiSelectValues(panel, msClass) {
  var el = panel ? panel.querySelector('.multi-select.' + msClass) : null;
  if (!el) return [];
...
function onMultiSelectChange(checkboxEl, label) {
  var el = checkboxEl ? checkboxEl.closest('.multi-select') : null;
  if (!el) return;
  var panel = _panelOf(el);
  var msClass = el.classList.contains('domain-ms') ? 'domain-ms' : 'criticality-ms';
  var vals = getMultiSelectValues(panel, msClass);
  ...
  applyFiltersInPanel(panel);
}

function applyFiltersInPanel(panel) {
  if (!panel) return;
  var selectedDomains = getMultiSelectValues(panel, 'domain-ms');
  var selectedCrits = getMultiSelectValues(panel, 'criticality-ms');
  var currentTarget = _currentTargetByPanel[panel.id] || '';

  var sections = panel.querySelectorAll('.issue-section');
  ...
  var rows = panel.querySelectorAll('.issue-row');
...
function clearAllFilters(linkEl) {
  var panel = _panelOf(linkEl);
  if (!panel) return;
  panel.querySelectorAll('.multi-select input[type=checkbox]').forEach(function(cb) { cb.checked = false; });
  var domainBtn = panel.querySelector('.multi-select.domain-ms .multi-select-btn');
  if (domainBtn) domainBtn.textContent = 'Domain';
  var critBtn = panel.querySelector('.multi-select.criticality-ms .multi-select-btn');
  if (critBtn) critBtn.textContent = 'Criticality';
  applyFiltersInPanel(panel);
}

function onTargetChange(selectEl) {
  var panel = _panelOf(selectEl);
  if (!panel) return;
  _runTargetChangeInPanel(panel, selectEl.value);
}

// Seeds a panel's initial state on load: stores its default target and runs the
// first target/effort/donut/filter pass scoped to that panel only.
function initAppCatPanel(panelId, defaultTarget) {
  var panel = document.getElementById(panelId);
  if (!panel) return;
  _runTargetChangeInPanel(panel, defaultTarget);
}

function _runTargetChangeInPanel(panel, targetId) {
  if (!panel) return;
  _currentTargetByPanel[panel.id] = targetId;
  var rows = panel.querySelectorAll('.issue-row[data-targets]');
  var totalEffort = 0;
  var domainCrits = {};
  ...
  var secSection = panel.querySelector('.issue-section[data-domain="security"]');
  if (secSection) {
  ...
  var effortEl = panel.querySelector('.effort-display');
  if (effortEl) effortEl.textContent = label + ' (total story points: ' + totalEffort + ')';

  updateDonuts(panel, domainCrits);
  applyFiltersInPanel(panel);
}

function updateDonuts(panel, domainCrits) {
  var container = panel ? panel.querySelector('.donut-container') : null;
  if (!container) return;
  container.innerHTML = '';
...
def _render_appcat_panel_fragment(report_dir, report_json_path, lang_token=None):
    """Render one language's AppCAT (Java/.NET) issues panel as a self-contained
    fragment for the merged report.

    lang_token ('java' / 'dotnet') is the authoritative language from the
    suffixed filename, passed by the discovery step. When None (legacy bare
    report.json), the language is inferred from the json's languages field.

    Returns a dict:
      tab_id        — unique panel/tab id (e.g. "tab-issues-java")
      tab_label     — tab button text ("Java Issues" / "DotNet Issues")
      tab_tooltip   — tab button tooltip
      fact_tabs     — list of (tab_id, label, tooltip, html) for this language's facts/
      panel_html    — inner HTML of the issues panel (app-info + summary + filters
                      + issue tables); NOT wrapped in the .tab-panel div
      init_script   — DOMContentLoaded snippet seeding this panel's state, or "".

    No HTML shell, lightbox, fact panels, footer, or MAIN_SCRIPT — the caller
    (single-language wrapper or multi-language composer) supplies those once.
    """
    ...
    # Language identity is authoritative from the suffixed filename (passed in);
    # only fall back to inferring from json content for the legacy bare report.json.
    if lang_token not in ("java", "dotnet"):
        lang_token = "dotnet" if is_dotnet else "java"
    is_dotnet = (lang_token == "dotnet")
    issues_tab_id = f"tab-issues-{lang_token}"
    tab_label = "DotNet Issues" if is_dotnet else "Java Issues"
    tab_tooltip = "Issues detected in the source code"
    ...
    # App info card (language-specific, lives inside this panel — mirrors C#
    # AppCatHtmlReportRenderer where each panel carries its own card).
    ...
    html.append(f'        <div class="app-info-row"><span class="app-info-label">Effort</span><span class="app-info-value effort-display">{escape(effort_display)}</span></div>')
    html.append('      </div>')
    html.append('      <div class="app-info-column">')
    ...
    if target_display_names and target_ids:
        ...
        html.append('  <label>Target Compute Service:</label>')
        html.append('  <select class="target-select" onchange="onTargetChange(this)">')
    ...
    html.append('  <div class="report-card-body">')
    html.append('    <div class="domain-summary-container donut-container">')
    ...
    if len(available_domains) > 1:
        html.append('  <div class="multi-select domain-ms">')
        html.append('    <button class="multi-select-btn" onclick="toggleMultiSelect(this)">Domain</button>')
        html.append('    <div class="multi-select-dropdown">')
        for did, dlabel in available_domains:
            html.append(f'      <label class="multi-select-option"><input type="checkbox" value="{escape(did)}" onchange="onMultiSelectChange(this,\'Domain\')"/> {escape(dlabel)}</label>')
        html.append('    </div>')
        html.append('  </div>')
    if len(available_crits) > 1:
        html.append('  <div class="multi-select criticality-ms">')
        html.append('    <button class="multi-select-btn" onclick="toggleMultiSelect(this)">Criticality</button>')
        html.append('    <div class="multi-select-dropdown">')
        for c in ["mandatory", "potential", "optional"]:
            if c in available_crits:
                html.append(f'      <label class="multi-select-option"><input type="checkbox" value="{c}" onchange="onMultiSelectChange(this,\'Criticality\')"/> {c.capitalize()}</label>')
        html.append('    </div>')
        html.append('  </div>')
    html.append('  <a class="clear-filters" href="javascript:void(0)" onclick="clearAllFilters(this)">Clear all</a>')
    html.append('</div>')
    ...
    def render_issue_section(section_title, groups, domain_id, badge_html=None, extra_rearchitect=None):
        ...
        for gi, (rule_id, incs) in enumerate(ordered):
            ...
            row_id = f"row-{lang_token}-{section_no_spaces}-{gi}"
        ...
        if extra_rearchitect:
            for ri, finding in enumerate(extra_rearchitect):
                row_id = f"row-{lang_token}-{section_no_spaces}-rearch-{ri}"
    ...
    if dotnet_upgrade_rules:
        ...
        for gi, (rule_id, rule_obj) in enumerate(ordered_du):
            ...
            severity = (rule_obj.get("severity") or "").strip().lower()
            story_point = rule_obj.get("effort", 0) or 0
            row_id = f"row-{lang_token}-dotnetupgrade-{gi}"
        ...
        if not rearchitect_in_labeled_upgrade and rearchitect_findings:
            ...
            for ri, finding in enumerate(rearchitect_findings):
                row_id = f"row-{lang_token}-{section_no_spaces.lower()}-rearch-{ri}"
                old_val = finding.get("old", "")
                explanation = finding.get("explanation", "")
                detected_in = finding.get("detectedIn", {})
                all_files = (detected_in.get("configFiles") or []) + (detected_in.get("sourceFiles") or [])
    ...
    if security_findings:
        ...
        for fi, finding in enumerate(ordered_sec):
            row_id = f"row-{lang_token}-security-{fi}"
    ...
    # Initial target change — seed this panel's state on load. Only emitted when
    # the report has selectable targets; without targets the server-rendered
    # effort/donut values stand (unchanged from prior behavior).
    init_script = ""
    if target_ids:
        init_script = f"<script>document.addEventListener('DOMContentLoaded', function() {{ initAppCatPanel('{issues_tab_id}', '{escape(target_ids[0])}'); }});</script>"

    return {
        "tab_id": issues_tab_id,
        "tab_label": tab_label,
        "tab_tooltip": tab_tooltip,
        "panel_title": component_display_name or component_name,
        "fact_tabs": fact_tab_data,
        "panel_html": '\n'.join(html),
        "init_script": init_script,
    }


def _render_html_shell(title, body_parts, init_scripts):
    """Wrap one or more panel fragments in the shared HTML shell (DOCTYPE, head,
    CSS, Mermaid, lightbox, MAIN_SCRIPT, footer). `body_parts` is the already-
    assembled inner markup (h1 + tab-nav + panels + fact panels); `init_scripts`
    is a list of per-panel DOMContentLoaded snippets appended after MAIN_SCRIPT."""
    out = []
    out.append('<!DOCTYPE html>')
    out.append('<html><head><meta charset="utf-8" />')
    out.append('<meta name="viewport" content="width=device-width, initial-scale=1" />')
    out.append(f'<title>Assessment - {escape(title)}</title>')
    out.append('<style>')
    out.append(CSS)
    out.append('</style>')
    out.append(MERMAID_HEAD_SCRIPT)
    out.append('</head><body>')

    # Lightbox (singleton overlay, shared across all panels)
    out.append('<div id="diagram-lightbox" class="diagram-lightbox" role="dialog" aria-modal="true">')
    out.append('  <button class="diagram-lightbox-close" id="diagram-lightbox-close" aria-label="Close">&#x2715;</button>')
    out.append('  <div class="diagram-lightbox-hint">Scroll to zoom &nbsp;&middot;&nbsp; Drag to pan &nbsp;&middot;&nbsp; Double-click to reset</div>')
    out.append('  <div class="diagram-lightbox-inner" id="diagram-lightbox-inner"></div>')
    out.append('</div>')

    out.append('<div class="main">')
    out.append(body_parts)
    out.append('<p class="footer">Your feedback is invaluable &mdash; <a href="https://aka.ms/ghcp-appmod/feedback">Share feedback</a></p>')
    out.append('<script>')
    out.append(MAIN_SCRIPT)
    out.append('</script>')
    for snippet in init_scripts:
        if snippet:
            out.append(snippet)
...
def generate_from_report_json(report_dir):
    """Generate report.html from report.json (Java/.NET) — single-language wrapper.

    Renders one AppCAT panel fragment and boxes it in the shared shell with a
    single-language tab nav (one issues tab + this language's fact tabs)."""
    report_json_path = os.path.join(report_dir, "report.json")
    frag = _render_appcat_panel_fragment(report_dir, report_json_path)

    body = []
    body.append(f'<h1>{escape(frag["panel_title"])}</h1>')
    body.append('<nav class="tab-nav">')
    body.append(f'  <button class="tab-btn active" data-tab="{frag["tab_id"]}" data-tooltip="{escape(frag["tab_tooltip"])}">Issues</button>')
    for tab_id, label, tooltip, _ in frag["fact_tabs"]:
        body.append(f'  <button class="tab-btn" data-tab="{tab_id}" data-tooltip="{escape(tooltip)}">{escape(label)}</button>')
    body.append('</nav>')
    body.append(f'<div id="{frag["tab_id"]}" class="tab-panel active">')
    body.append(frag["panel_html"])
    body.append(f'</div><!-- #{frag["tab_id"]} -->')
    for tab_id, _, _, content in frag["fact_tabs"]:
        body.append(f'<div id="{tab_id}" class="tab-panel">')
        body.append(f'  <div class="fact-content">{content}</div>')
        body.append('</div>')

    return _render_html_shell(frag["panel_title"], '\n'.join(body), [frag["init_script"]])


def _render_jsts_panel_fragment(md_path):
    """Render the JS/TS report as a self-contained panel fragment. JS/TS has no
    issues table, filters, or target dropdown — its panel is a static markdown
    blob — so there is no per-panel state and no init_script."""
    ...
    return {
        "tab_id": "tab-issues-javascript-typescript",
        "tab_label": "JavaScript/TypeScript",
        "tab_tooltip": "Outdated npm packages and their available updates",
        "panel_title": title,
        "fact_tabs": [],
        "panel_html": f'<div class="fact-content">{fact_html}</div>',
        "init_script": "",
    }


def generate_from_js_markdown(report_dir):
    """Generate report.html from js-assessment-report.md (JS/TS) — single-language
    wrapper. Locates the markdown, renders its fragment, boxes it in the shell."""
    ...
    frag = _render_jsts_panel_fragment(md_path)
...
def _discover_language_inputs(report_dir):
    """Find each language's primary report file in report_dir.

    Returns an ordered list of (kind, path, lang_token) where kind is 'appcat'
    (report-java.json / report-dotnet.json) or 'jsts'
    (report-javascript-typescript.md). lang_token ('java' / 'dotnet') is the
    AUTHORITATIVE language taken from the suffixed filename — never inferred from
    the json's languages field, which is often missing or 'N/A'. It is None for
    jsts panels and for the legacy bare-name fallback (report.json), where the
    appcat renderer falls back to inferring the language from json content.
    Mirrors the sibling generate_report_md.py discovery."""
    suffixed = [
        ("appcat", "report-java.json", "java"),
        ("appcat", "report-dotnet.json", "dotnet"),
        ("jsts", "report-javascript-typescript.md", None),
    ]
    found = [(kind, os.path.join(report_dir, name), lang_token)
             for (kind, name, lang_token) in suffixed
             if os.path.isfile(os.path.join(report_dir, name))]
    if found:
        return found

    # Legacy single-language fallback: bare filenames (lang_token None → infer).
    bare_json = os.path.join(report_dir, "report.json")
    if os.path.isfile(bare_json):
        return [("appcat", bare_json, None)]
    bare_ncu = os.path.join(report_dir, "js-assessment-report.md")
    if not os.path.isfile(bare_ncu):
        parent_ncu = os.path.join(os.path.dirname(report_dir), "js-assessment-report.md")
        if os.path.isfile(parent_ncu):
            bare_ncu = parent_ncu
    if os.path.isfile(bare_ncu):
        return [("jsts", bare_ncu, None)]
    return []


def _compose_html(report_dir, inputs):
    """Render N language fragments into ONE report.html: a flat tab-nav (one
    issues tab per language, first active) followed by the shared fact tabs
    (rendered once from the first language that carries facts), each fragment
    wrapped in its own .tab-panel. Mirrors C# MergedRepoHtmlComposer."""
    fragments = []
    for kind, path, lang_token in inputs:
        if kind == "appcat":
            fragments.append(_render_appcat_panel_fragment(report_dir, path, lang_token))
        else:
            fragments.append(_render_jsts_panel_fragment(path))

    if not fragments:
        print("Error: No language report found.", file=sys.stderr)
        sys.exit(1)

    # Facts are shared, not per-language: take the first non-empty fact set so
    # the fact tabs render exactly once after all language panels.
    shared_fact_tabs = next((f["fact_tabs"] for f in fragments if f["fact_tabs"]), [])

    page_title = fragments[0]["panel_title"]

    body = []
    body.append(f'<h1>{escape(page_title)}</h1>')

    # Tab nav: one issues button per language (first active) + shared fact buttons.
    body.append('<nav class="tab-nav">')
    for i, frag in enumerate(fragments):
        active = " active" if i == 0 else ""
        body.append(f'  <button class="tab-btn{active}" data-tab="{frag["tab_id"]}" data-tooltip="{escape(frag["tab_tooltip"])}">{escape(frag["tab_label"])}</button>')
    for tab_id, label, tooltip, _ in shared_fact_tabs:
        body.append(f'  <button class="tab-btn" data-tab="{tab_id}" data-tooltip="{escape(tooltip)}">{escape(label)}</button>')
    body.append('</nav>')

    # Language panels (first active).
    for i, frag in enumerate(fragments):
        active = " active" if i == 0 else ""
        body.append(f'<div id="{frag["tab_id"]}" class="tab-panel{active}">')
        body.append(frag["panel_html"])
    ...
    # Shared fact panels (rendered once).
    for tab_id, _, _, content in shared_fact_tabs:
        body.append(f'<div id="{tab_id}" class="tab-panel">')
        body.append(f'  <div class="fact-content">{content}</div>')
        body.append('</div>')

    init_scripts = [f["init_script"] for f in fragments]
    return _render_html_shell(page_title, '\n'.join(body), init_scripts)
...
def main():
    ...
    inputs = _discover_language_inputs(report_dir)
    if not inputs:
        print("Error: No language report found (report-java.json / report-dotnet.json / "
              "report-javascript-typescript.md, or legacy report.json / js-assessment-report.md).",
              file=sys.stderr)
        sys.exit(1)

    output = _compose_html(report_dir, inputs)
