...
def main():
    ...
    try:
        ...
        # When the caller passes --report-id, all languages of a multi-language unit
        # share one report-{id}/ directory and Java writes its data as the
        # language-suffixed report-java.json (so sibling report-dotnet.json etc. can
        # coexist). Without it, keep the single-language behaviour: derive the id from
        # the report and write the bare report.json.
        ...
        final_report_name = "report-java.json" if coordinated else "report.json"
        final_report = os.path.join(versioned_dir, final_report_name)
