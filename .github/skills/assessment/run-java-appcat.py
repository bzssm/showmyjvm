...
def clean_old_reports(reports_dir, keep=None):
    ...
    # `keep`, when given, is the absolute path of one report-* directory that must
    # be preserved — used in coordinated multi-language mode where that shared
    # directory may already hold a sibling language's output.
...
def main():
    ...
    try:
        ...
        # `--report-id` (coordinated mode) means a multi-language unit shares ONE
        # report-{id}/ directory: use the caller's id and preserve any sibling
        # language's output already in that directory. Without it, derive the id from
        # this report and own the directory exclusively. Either way the Java primary
        # report is ALWAYS written as the language-suffixed report-java.json — there is
        # no bare report.json, so the report directory only ever holds report-{lang}.* files.
        coordinated = bool(values.report_id and values.report_id.strip())
        report_id = values.report_id.strip() if coordinated else derive_report_id(produced_report)
        ...
        # In coordinated mode the shared report-{id}/ may already hold a sibling
        # language's output (report-dotnet.json, report-javascript-typescript.md),
        # so preserve it — only clean stale OTHER report-* dirs, never this one.
        try:
            clean_old_reports(reports_dir, keep=versioned_dir if coordinated else None)
        except Exception as exc:  # noqa: BLE001
            log(f"Stale-report cleanup failed ({exc}); continuing with new report.")
        os.makedirs(versioned_dir, exist_ok=True)
        final_report = os.path.join(versioned_dir, "report-java.json")
