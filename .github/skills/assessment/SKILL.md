...
- `report-id` (optional): A pre-established versioned-report id (a `yyyyMMddHHmmss` timestamp). When provided, **all** languages of this unit write into the **one** shared `report-{report-id}/` directory, each saving its data under a **language-suffixed** filename (`report-java.json` / `report-dotnet.json` / `report-javascript-typescript.md`) so several languages coexist in a single directory. When omitted, derive the id per language as before (single-language behavior) and use the bare filename. Always pair `report-id` with `language` — one call assesses one language but targets the shared directory.
...
3. **Save Report to Versioned Directory (All languages)**:
   - **For Java projects**: The `run-java-appcat.py` script automatically writes the report into the versioned directory — no manual saving needed. With `--report-id` it writes `report-{report-id}/report-java.json` (shared directory, suffixed name); without it, it derives the `reportId` from `metadata.analysisStartTime` and writes the bare `report-{reportId}/report.json`.
   - **For .NET projects**:
     ...
     4. Move the report to `{workspace-path}/.github/modernize/assessment/reports/report-{reportId}/report-dotnet.json` when a `report-id` was provided (shared directory, suffixed name); otherwise to `report-{reportId}/report.json`
...
Report location depends on project type. The primary filename depends on whether a `report-id` was provided: with it, the language-suffixed name is used in the shared directory; without it, the bare name is used.

**For Java projects** (direct execution via `run-java-appcat.py`):
- Saved to versioned directory: `{workspace-path}/.github/modernize/assessment/reports/report-{reportId}/report-java.json` (suffixed, shared) or `report.json` (bare, single-language)

**For .NET projects** (direct execution):
- Initially generated at: `{workspace-path}/.github/modernize/appcat/result/report.json`
- Moved to versioned directory: `{workspace-path}/.github/modernize/assessment/reports/report-{reportId}/report-dotnet.json` (suffixed, shared) or `report.json` (bare, single-language)
...
- ✅ **For Java and .NET**: Report generated at `{workspace-path}/.github/modernize/assessment/reports/report-{reportId}/` as `report-java.json` / `report-dotnet.json` (shared) or `report.json` (single-language)
